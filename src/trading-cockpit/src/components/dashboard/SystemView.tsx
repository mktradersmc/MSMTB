                ...
                <div className="max-w-3xl space-y-6">
                    ...
                    <section className="border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden bg-white dark:bg-slate-900">
                        <div className="bg-slate-50 dark:bg-slate-900/50 px-4 py-2 border-b border-slate-200 dark:border-slate-800 text-xs font-bold text-slate-500 uppercase tracking-wider flex justify-between items-center">
                            ...
                            <span className="text-[10px] bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 px-1.5 py-0.5 rounded">NT8</span>
                        </div>
                        <div className="p-4">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-md bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center border border-indigo-100 dark:border-indigo-500/10">
                                        <Monitor className="text-indigo-600 dark:text-indigo-400" size={16} />
                                    </div>
                                    <div>
                                        <h3 className="font-medium text-slate-900 dark:text-slate-200 text-sm">NinjaTrader Controller</h3>
                                        <p className="text-slate-500 text-xs">Start NT8 and perform auto-login.</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-[1fr,1fr,auto] gap-3 items-end">
                                <div>
                                    <label className="block text-[10px] font-medium text-slate-500 uppercase mb-1">Username</label>
                                    <input
                                        type="text"
                                        id="nt_user"
                                        placeholder="Username"
                                        className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded px-2 py-1.5 text-xs outline-none focus:border-indigo-500 text-slate-900 dark:text-white"
                                    />
                                </div>
                                <div>
                                    <label className="block text-[10px] font-medium text-slate-500 uppercase mb-1">Password</label>
                                    <input
                                        type="password"
                                        id="nt_pass"
                                ...
                                <button
                                    onClick={async () => {
                                        const user = (document.getElementById('nt_user') as HTMLInputElement).value;
                                        const pass = (document.getElementById('nt_pass') as HTMLInputElement).value;
                                        if (!user || !pass) {
                                            alert("Please enter credentials");
                                            return;
                                        }
                                        try {
                                            const res = await fetchDirect('/api/admin/ninjatrader/start', {
                                                method: 'POST',
                                                headers: { 'Content-Type': 'application/json' },
                                                body: JSON.stringify({ username: user, password: pass })
                                            });
                                            const d = await res.json();
                                            if (d.success) alert("NinjaTrader Started! " + d.message);
                                            else alert("Error: " + d.error);
                                        } catch (e) {
                                            alert("Network Error");
                                        }
                                    }}
                                    className="h-[30px] px-3 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-medium rounded transition-colors flex items-center gap-1.5 whitespace-nowrap"
                                >
                                    <Zap size={14} /> Start & Only Login
                                </button>
                            </div>
                        </div>
                    </section>
