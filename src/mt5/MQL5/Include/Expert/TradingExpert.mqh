...
public:
   ...
   // Active Aggregation States
   double activeVolume;
   double activeSwap;
   double activeProfit;
   double activeComm;
   double activeOpenWeightedSum;
   ...
   void ResetActiveState() {
      activeVolume = 0; activeSwap = 0; activeProfit = 0;
      activeComm = 0; activeOpenWeightedSum = 0;
      activeCurrentPrice = 0; activeTime = 0; activeTicket = 0;
   }
...
// PROXY MODE Removed
    ...
    for(int i=0; i<total; i++)
    {
       ...
       if(PositionSelectByTicket(ticket))
       {
          ...
          for(int k=0; k<m_metrics.Total(); k++) {
             CMetric* m = m_metrics.At(k);
             if((m.magic == magic && magic != 0) || (magic == 0 && m.positionTicket == ticket)) { metric = m; break; }
          }
          ...
          metric.activeVolume += pVol;
          metric.activeOpenWeightedSum += (pVol * pOpen);
          metric.activeProfit += PositionGetDouble(POSITION_PROFIT);
          metric.activeSwap += PositionGetDouble(POSITION_SWAP);
          metric.activeComm += PositionGetDouble(POSITION_COMMISSION);
          metric.activeCurrentPrice = PositionGetDouble(POSITION_PRICE_CURRENT);
          
          ulong pTime = (ulong)PositionGetInteger(POSITION_TIME);
          if (metric.activeTime == 0 || pTime < metric.activeTime) {
              metric.activeTime = pTime;
              metric.activeTicket = ticket; // use oldest ticket as main
              metric.sl = PositionGetDouble(POSITION_SL);
              metric.tp = PositionGetDouble(POSITION_TP);
          }
    ...
    for(int k=0; k<m_metrics.Total(); k++) {
        CMetric* metric = m_metrics.At(k);
        if(!metric.isOpen) continue;
        
        CJAVal positionObj;
        ulong magic = metric.magic;
        
        if (magic == 0) positionObj["id"] = "manual_" + IntegerToString(metric.activeTicket);
        else positionObj["id"] = IntegerToString(magic);
        
        positionObj["ticket"] = (long)metric.activeTicket;
        positionObj["time"] = (long)metric.activeTime;
        
        positionObj["symbol"] = metric.symbol;
        positionObj["type"] = metric.type;
        positionObj["vol"] = NormalizeDouble(metric.activeVolume, 2);
        
        double avgOpen = metric.activeVolume > 0 ? NormalizeDouble(metric.activeOpenWeightedSum / metric.activeVolume, 5) : 0;
        positionObj["open"] = avgOpen;
        positionObj["current"] = metric.activeCurrentPrice;
        positionObj["sl"] = metric.sl;
        positionObj["tp"] = metric.tp;
        
        positionObj["swap"] = metric.activeSwap;
        positionObj["profit"] = metric.activeProfit;
        positionObj["commission"] = metric.activeComm;
        ...
        positionObj["metrics"]["openTime"] = (long)metric.activeTime;
        positionObj["metrics"]["entryPrice"] = avgOpen;
        positionObj["comment"] = metric.comment;
        
        // Print("[TradingExpert] EV_TRADE_UPDATE Aggregated: ", positionObj.Serialize());
...
void CTradingExpert::ExecuteTrade(CJAVal &data, CJAVal &response)
{
   ...
   while(remainingVolume >= brokerMin)
   {
      ...
      // Normalize to Step (Adding epsilon to prevent floating point floor issues)
      currentVol = MathFloor((currentVol + brokerStep * 0.00001) / brokerStep) * brokerStep;
