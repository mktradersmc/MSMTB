...
export const ReplayToolbar: React.FC = () => {
    ...
    const [speedMs, setSpeedMs] = useState(500); // Defaults to 500ms delay
    ...
    useEffect(() => {
        ...
        if (isAutoPlaying && activeSession) {
            timer = setTimeout(() => {
                ...
                const stepMs = stepSizeMin * 60000;
                const currentTime = activeSession.simulation_time;
                // Add 1ms to ensure we always step strictly into the *next* boundary
                const targetTime = Math.ceil((currentTime + 1) / stepMs) * stepMs;
                const amountMs = targetTime - currentTime;

                stepForward(amountMs, symbols);
            }, speedMs);
        }
    ...
    // E.g., Slider max 20 signifies "fastest" (lowest delay, e.g. 50ms), min 1 signifies "slowest" (e.g. 1000ms delay)
    // Here we'll make the slider map from 1x to 20x.
    // Value 1 = 1000ms, Value 20 = 50ms
    const handleSpeedChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const val = parseInt(e.target.value);
        // Map 1-20 straight to milliseconds (Reverse correlation)
        // val 1 -> 1000ms
        // val 20 -> 50ms
        // Lerp: ms = 1000 - ((val - 1) * (950 / 19))
        const delay = 1000 - ((val - 1) * (950 / 19));
        setSpeedMs(Math.max(50, Math.round(delay)));
    };

    // Calculate slider value for derived state
    const sliderValue = Math.round(1 + ((1000 - speedMs) * 19) / 950);
