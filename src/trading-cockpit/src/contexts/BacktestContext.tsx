...
export const BacktestProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [isCutModeActive, setIsCutModeActive] = useState<boolean>(false);
    const [jumpVersion, setJumpVersion] = useState<number>(0);
    ...
    useEffect(() => {
            ...
            if (payload && payload.sessionId) {
                setActiveSession(prev => {
                    if (prev && prev.id === payload.sessionId) {
                        return { ...prev, current_balance: payload.balance };
                    }
                    return prev;
                });
            }
        ...
        socket.on('backtest_balance_update', onBalanceUpdate);

        return () => {
            socket.off('backtest_balance_update', onBalanceUpdate);
        };
    }, []);
